...
void GameScreen::OnPhaseChanged(GamePhase newPhase) {
    ...
    Logger::Info(std::string("[DEBUG] Phase changed to: ") + std::to_string(static_cast<int>(newPhase)));
}
...
void GameScreen::UpdateInputModeBasedOnGameState() {
    ...
    if (gameState_->inGameStep == 0 && gameState_->isMyLeader && gameState_->myTeam == gameState_->currentTurn) {
        ...
        inInputMode_ = true;
        Logger::Info("[DEBUG] Auto-entered HINT input mode (leader turn)");
        ...
        inInputMode_ = true;
        Logger::Info("[DEBUG] Auto-entered ANSWER input mode (team member turn)");
    } else {
        // 그 외: 입력 모드 해제
        inputMode_ = NONE;
        inInputMode_ = false;
        Logger::Info("[DEBUG] Input mode disabled (not my turn)");
    }
    needsRedraw_ = true;
}
