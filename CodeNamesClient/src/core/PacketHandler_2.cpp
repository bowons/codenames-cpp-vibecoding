#include "../../include/core/PacketHandler_2.h"
#include "../../include/core/PacketProtocol.h"
#include "../../include/core/GameState_2.h"
#include <iostream>
#include <sstream>
#include <algorithm>
...
void PacketHandler::ProcessPacket(const std::string& packet) {
    ...
    if (delimiterPos != std::string::npos) {
        // 없다면 데이터가 없는 패킷 (예: "ERROR", "INVALID_TOKEN")
    ...
    // 이후 이에 맞는 헨들러를 호출한다.
        try {
        it->second(packetData); // second - callback 함수
...
// 아래 내용들은 핸들러로 gameState를 적절하게 갱신하여
// 이를 GameStateObserver에게 알려 GUI에 반영되도록 프로그래밍 된 것 같다.
